...
<body>
			...
			<div style="padding: 15px;">
				...
				<form class="layui-form layui-form-pane">
					...
					<div class="layui-form-item layui-form-text">
						...
						<div class="layui-input-block">
							<textarea name="sumary" id="sumary" placeholder="请输入酒店介绍" required
						...
						<div class="layui-tab layui-tab-card" lay-filter="demo"
							...
							<label class="layui-form-label">房间类型
								<button style="margin-left: 10px" type="button"
									data-type="tabAdd"
									class="layui-btn  site-demo-active layui-btn-xs" id="test2">
	...
	<script>
		...
		function getInitTabContent(i,img,name,money,remark) {
			...
			str += '<button onclick="onc(' + i
					+ ')" type="button" class="layui-btn" id="btn' + i
					+ '">上传图片</button>';
			...
			str += '<input value="'+name+'" type="text" id="n'+i+'" name="n'+i+'" required lay-verify="required" placeholder="房间名称" autocomplete="off" class="layui-input">';
			str += '<input value="'+money+'" oninput = "value=value.replace(/[^\d]/g,"")" type="text" id="m'+i+'" name="m'+i+'" required lay-verify="required" placeholder="房间价格" autocomplete="off" class="layui-input">';
			str += '<input value="'+remark+'" type="text" id="s'+i+'" name="s'+i+'" required lay-verify="required" placeholder="房间介绍" autocomplete="off" class="layui-input">';
		...
		layui.use('form', function() {
			...
			form.on('submit(formDemo)', function(data) {
				if(imgname == ''){
					...
					data.field.hid = hid;
					data.field.imgname=imgname;
					data.field.holtelSize = bz_txt.length;
					for ( var i = 1; i <= bz_txt.length; i++) {
						data.field['img' + i] = bz_img[i - 1];
					}
					$.post("/TravelService/CanHoltel?action=update",
