...
<body>
					...
					<div class="layui-form-item">
						...
						<div class="layui-input-block">
							<input oninput = "value=value.replace(/[^\d]/g,'')" type="text" name="money" id="money" required lay-verify="required"
								placeholder="请输入门票价格" autocomplete="off" class="layui-input">
						</div>
					</div>
	...
	<script>
		...
		var ci = $.parseJSON(localStorage.getItem("ci"));
		document.getElementById('money').value = ci.money;
		document.getElementById('loc').value = ci.loc;
		document.getElementById('date').value = ci.date;
		document.getElementById('time').value = ci.time;
		...
		layui.use('form', function() {
			...
			$.get("/JuchangService/theatre/search", function(data,
				...
				if (data_json.code == 0) {
					...
					document.getElementById('tid').value = ci.tid;
					form.render('select');
				}
			});
			...
			$.get("/JuchangService/showplace/search", function(data,
				...
				if (data_json.code == 0) {
					...
					document.getElementById('sid').value = ci.sid;
					form.render('select');
				}
			});
			...
			form.on('submit(formDemo)', function(data) {
					data.field.cid=ci.cid;
					$.post("/JuchangService/ci/update",
